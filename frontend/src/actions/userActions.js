...
export const getUserBalance = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`/api/users/balance`, config);
    dispatch({
      type: USER_BALANCE_SUCCESS,
      payload: data,
    });
...
export const checkDepositState = () => async (dispatch, getState) => {
  try {
    const {
      userLogin: { userInfo },
    } = getState();
    ...
    const { data } = await axios.get(`/api/users/checkdeposit`, config);
    dispatch({
      type: USER_BALANCE_SUCCESS,
      payload: data,
    });
...
export const withdraw = (params) => async (dispatch, getState) => {
  try {
    dispatch({
    ...
    const { data } = await axios.put(`/api/users/withdraw`, params,config);
    dispatch({
